...
az network front-door create -n $frontDoor -g $rg --tags $tags --backend-address "$app1.azurewebsites.net" --accepted-protocols Http Https

# Add second Backend
az network front-door backend-pool backend add -g $rg --front-door-name $frontDoor --address "$app2.azurewebsites.net" --pool-name 'DefaultBackendPool'
